<template>
  <div class="store-search">
    ...
    <header class="store-header">
      <h1 class="store-title store-title--makro">Makro <span>Search</span></h1>
      <form class="search-form" @submit.prevent="doSearch">
        <input v-model="query" type="text" class="search-input" placeholder="Search Makro products..." autofocus />
        <button type="submit" class="search-btn search-btn--makro" :disabled="searching || !query.trim()">
          {{ searching ? '...' : 'Search' }}
        </button>
      </form>
    </header>

    <div v-if="searching" class="status-msg">Searching Makro...</div>
    ...
    <div class="product-grid">
      <a v-for="p in products" :key="p.id" :href="p.link" target="_blank" rel="noopener" class="product-card">
        ...
        <div class="card-body">
          ...
          <p v-if="p.nameEn && p.nameEn !== p.name" class="card-name-en">{{ p.nameEn }}</p>
          <div class="card-pricing">
            <span class="card-price">{{ formatPrice(p.perUnitPrice) }}</span>
            <span v-if="p.unitFactor > 1" class="card-pack">/unit</span>
            <span v-if="p.originalPrice > p.price" class="card-original">{{ formatPrice(p.originalPrice) }}</span>
          </div>
          <p v-if="p.unitFactor > 1" class="card-pack-info">Pack of {{ p.unitFactor }} = {{ formatPrice(p.price) }}</p>
...
interface Product {
  ...
  originalPrice: number; discount: string; image: string; link: string
}
...
async function doSearch() {
  ...
  try {
    const data = await $fetch<any>('/api/makro/search', { method: 'POST', body: { q: query.value, limit: 40 } })
    products.value = (data.hits || []).map((hit: any) => {
      const doc = hit.document || hit
      const images = doc.images || []
      const price = doc.displayPrice || 0
      const uf = doc.unitFactor || 1
      const orig = doc.originalPrice || 0
      return {
        id: doc.id || '',
        name: doc.title || '',
        nameEn: doc.titleEn || '',
        ...
        price,
        perUnitPrice: uf > 1 ? Math.round((price / uf) * 100) / 100 : price,
        unitFactor: uf,
        originalPrice: orig,
        discount: orig > price ? `-${Math.round(((orig - price) / orig) * 100)}%` : '',
        image: images[0] || '',
        link: doc.makroId ? `https://www.makro.pro/th/p/${doc.makroId}-${doc.id}` : '#',
      }
...
.store-title--makro span { color: #0055a5; }
[data-theme="dark"] .store-title--makro span { color: #60a5fa; }
...
.search-btn--makro { background: #0055a5; }
...
.card-name-en { font-size: 0.65rem; color: var(--text-muted); margin: 0; display: -webkit-box; -webkit-line-clamp: 1; -webkit-box-orient: vertical; overflow: hidden; }
...
.card-pack { font-size: 0.65rem; color: var(--text-muted); }
.card-original { font-size: 0.65rem; color: var(--text-muted); text-decoration: line-through; }
.card-pack-info { font-size: 0.6rem; color: var(--text-secondary); margin: 0; }
.card-brand { font-size: 0.65rem; color: var(--text-secondary); margin: 0; margin-top: auto; }
</style>
